...
public class Chapter06_30 {
   ...
   *       int add(int b, int a) {...}
   *
   * 위의 두 메서드는 매개변수의 이름만 다를 뿐 매개변수의 타입이 같기 때문에 오버로딩이 성립하지 않는다. 매개변수의 이름이 다르면
   * 메서드 내에서 사용되는 변수의 이름이 달라질 뿐, 아무런 의미가 없다. 그래서 이 두 메서드는 정확히 같은 것이다. 마치 수학에서
   * 'f(x) = x + 1' 과 'f(a) = a + 1' 이 같은 표현인 것과 같다.
   *
   * 만약 저 코드를 컴파일 한다면 'add(int, int) is already fined.' 라는 에러 메세지가 나타날 것이다.
   *
   *  ex2) int add(int a, int b) {...}
   *       long add(int a, int b) {...}
   *
   * 이번 경우는 리턴타입만 다른 경우이다. 매개변수의 타입과 개수가 일치하기 때문에 add(3, 3) 과 같이 호출하였을 때 어떤 메서드가 호출될 것인지
   * 결정할 수 없기 때문에 오버로딩으로 간주되지 않는다.
   *
   * 이 경우 역시 컴파일 시 에러가 발생한다.
   *
   *
   *  ex3) long add(int a, long b) {...}
   *       long add(long a, int a) {...}
   *
   * 두 메서드 모두 int 형과 long 형 매개변수가 하나씩 선언되어 있지만, 서로 순서가 다른 경우이다. 이 경우에는 호출 시 매개변수의 값에 의해
   * 호출될 메서드가 구분될 수 있으므로 중복된 메서드 정의가 아닌, 오버로딩으로 간주한다.
   *
   * 하지만 이처럼, 단지 매개변수의 순서만을 다르게 하여 오버로딩을 구현하면, 사용자가 매개변수의 순서를 외우지 않아도 되는 장점이 있지만 단점도 존재한다.
   * 예를 들어 add(3, 3L) 와 같이 호출하면 첫번째 메서드가, add(3L, 3) 과 같이 호출하면 두번째 메서드가 호출된다. 단, 이 경우에는 add(3,3) 과 같이
   * 호출할 수 없다. 이와 같이 호출할 경우 두 메서드 중 어느 것이 호출되어야 하는 것인지 알 수 없어 에러가 발생한다.
   * **/
